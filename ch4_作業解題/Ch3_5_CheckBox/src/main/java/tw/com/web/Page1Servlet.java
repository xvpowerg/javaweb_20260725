...
public class Page1Servlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<String> errorFoods = new ArrayList();
		...
		for (String fId : foods) {
			...
			String countStr = req.getParameter("count_"+fId);
			int count = 0;
			try {
			...
			if (count == 0) {
				errorFoods.add(f.getName());
				continue;
			}
			
			int itemPrice =  count * f.getPrice();
			total += itemPrice;
			out.println("<li>"+f.getName()+":"+f.getPrice()+" 數量:"+countStr+ "金額:"+itemPrice+" </li>");
			
		}
		if(errorFoods.size() != 0) {
			req.setAttribute("error_foods", errorFoods);
